...
def compute_dice_coefficients(mask_gts, mask_preds):
  """Compute soerensen-dice coefficient.
  ...
  """
  dscs = []
  for b in range(mask_gts.shape[0]):
    dsc = compute_dice_coefficient(mask_gts[b], mask_preds[b])
    dscs.append(dsc)
  return torch.tensor(dscs)
