...
# set up parser
parser = argparse.ArgumentParser("MedSedX OOD evaluating", add_help=False)
    ...
    with torch.no_grad():
        for data, label in pbar:
            ...
            for idx, name in enumerate(data["name"]):
                file = name.replace(f"/path/to/MedSegDB/{args.data_dim}/", "").replace("npy_gts", "npy_imgs")
...
def main(args):
    ...
    evaluation = args.shift_type[6:]
    print(f"save evaluation result to {join(work_dir, 'OOD_{}{}.md'.format(evaluation, epoch))}")
    with open(join(work_dir, 'OOD_{}{}.md'.format(evaluation, epoch)), mode="w") as f:
        f.write(f"# OOD {evaluation} evaluation\n\n")
        ...
        for task in sorted(os.listdir(data_path)):
            ...
            for dataset in sorted(os.listdir(task_path)):
                ...
                if 'inference' in os.listdir(dataset_path):
                    ...
                    # evaluate
                    result_task = evaluate(msx_model, seg_metric, test_dataloader, 
                                           img_size, img_transform, box_transform, 
                                           dataset, task, result_total=result_total, 
                                           meta=meta, args=args)
                    result_task = ", ".join([f"{k.upper()} ({v:.4f})" for k, v in result_task.items()])
                    ...
                    for sequence in sorted(os.listdir(dataset_path)):
                        ...
                        metric_task = evaluate(msx_model, seg_metric, test_dataloader, 
                                               ...
                                               meta=meta, args=args)
                        metric_task = ", ".join([f"{k.upper()} ({v:.4f})" for k, v in metric_task.items()])
                        f.write(f"    - {sequence}: {metric_task}\n")
    ...
    df.to_csv(f"{work_dir}/OOD_{evaluation}{epoch}.csv", index=False)
